...
regularly uses \emph{'n} as determiner and while in Dutch this word is used in speech, it is 
...
due to this: \emph{preciese} `exact, precise' is an adjective modifying \emph{grens} `border'. While there is a pattern
...
transfer errors. \emph{om te} is a phrase that prefaces infinitives in both Afrikaans and Dutch
However, it is the case that it is not automatically correct in Dutch, if \emph{om te} is used in
Afrikaans. Example \ref{ex:exother1} is an example of an incorrect Dutch translation 
if \emph{om te} is kept. The fact that \emph{om te} is seperable, makes this problem
even harder to solve.

\ex. \label{ex:exother1}
    In beginsel kan jy aan Wikipedia meewerk sonder om enige besprekingsblaaie te lees. \\
    In principe kan jij aan Wikipedia *meewerk zonder {\em om} enig *besprekingsblaaie {\em te} lezen. \\
    In principe kan jij aan Wikipedia meewerken zonder enige besprekingsbladen te lezen. \\
    `In principle you can work on Wikipedia without reading any talkpages.'
