INPUT=/tmp/af-nl.gentest.postchunk
DEV=/home/fran/source/apertium/incubator/apertium-af-nl/dev/bidix/
...
cat $INPUT | grep '<vblex>' | grep '@' | cut -f2 -d'@' | cut -f1 -d'<' | lt-proc -w $DEV/../../af-nl.automorf.bin  | cg-proc /tmp/temp_cg.bin  | cut -f2- -d'/' | sed 's/<pres><p3><sg>//g' | grep '<vblex>' | cut -f1 -d'$' | sort -f | uniq -c | sort -gr  | grep -v '[0-9] $' > $DEV/pending_verbs.txt
